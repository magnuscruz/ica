...
public class Gnuplot {
	...
	protected static final ResourceBundle config = ResourceBundle.getBundle("config");
	/**
	...
	/**
	 * The font of the plot. If this variable is <code>null</code> (default),
	 * no title is used.
	 *
	 */
	protected String fontLabel;
	...
	public String getGnuplotString() {
		...
		try {
			...
			while (it.hasNext()) {
				...
				if (t!=null && t.indexOf(XYSeriesChart.PROTOTIPOS) > -1) {
					s += " pointtype 6 ";
				}
				s += ",";
			}
			s = s.substring(0, s.length() - 1);
			if (style != null && style.indexOf("boxes") > -1)
				buf.append("set boxwidth 0.9\n");
			buf.append(s + "\n");
	...
	public void run() {
		try {
			String gnuplotString = getGnuplotString();
			...
			FileOutputStream w = new FileOutputStream(demo);*/
			//System.out.println(gnuplotString);
			//w.write(gnuplotString.getBytes());
			GnuplotCall.call(gnuplotString, false, true);
			if (!fig) {
				String command = config.getString("epstopdf.bin")
											+ " "
											+ outputfile
											+ ".eps";
				//System.out.println(command);
				//w.write(command.getBytes());
				Process p =
